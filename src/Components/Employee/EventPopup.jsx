...
const EventPopup = ({ onClose, eventData }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-start p-10 z-50">
      <div
        className={`bg-white p-6 rounded-lg shadow-lg w-full max-w-2xl ${popupClass}`}
      >
        <button
          onClick={onClose}
          className="absolute top-2 right-2 text-xl text-gray-700 font-bold hover:text-gray-500"
        >
          X
        </button>
        ...
        <p className="text-md text-gray-600">{eventData.description}</p>
      </div>
    </div>
  );
};
